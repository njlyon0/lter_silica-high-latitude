...
for(place in c("Yukon")) {
  ...
  sizer_ggplot_test <- function(raw_data = NULL, x = NULL, 
                                y = NULL, sizer_data = NULL, 
            trendline = "none", vline = "all", 
            sharp_colors = c("#DDDDDD", "orange")) {
    groups <- slope_type <- NULL
    if (is.null(raw_data) | is.null(sizer_data) | is.null(x) | 
        is.null(y)) 
      stop("All arguments must be provided")
    if (class(raw_data) != "data.frame" | class(sizer_data) != 
        "data.frame") 
    ...
    p <- ggplot2::ggplot(data = raw_data, ggplot2::aes_string(x = x, 
                                                              y = y)) + ggplot2::theme_classic()
    if (vline %in% c("all", "changes")) {
      p <- p + ggplot2::geom_vline(xintercept = sizer_data$mean_x, 
                                   color = "orange", linetype = 2, na.rm = TRUE) + ggplot2::geom_vline(xintercept = sizer_data$x_grid, 
                                                                                                       color = "orange", linetype = 2, na.rm = TRUE)
    }
    if (!base::all(is.na(sizer_data$pos_to_neg)) & vline %in% 
        c("all", "inflections")) {
      p <- p + ggplot2::geom_vline(xintercept = sizer_data$pos_to_neg, 
                                   color = "blue", na.rm = TRUE)
    }
    if (!base::all(is.na(sizer_data$neg_to_pos)) & vline %in% 
        c("all", "inflections")) {
      p <- p + ggplot2::geom_vline(xintercept = sizer_data$neg_to_pos, 
    ...
    if (trendline == "sharp") {
      p <- p + ggplot2::geom_smooth(ggplot2::aes(group = groups, 
                                                 color = slope_type), method = stats::lm, formula = "y ~ x", 
                                    se = FALSE) + 
        ggplot2::theme(legend.position = "none")
    }
    
    # If trendline is sharp but inflections exist, use only second color
    if(trendline == "sharp" & !base::all(is.na(sizer_data$pos_to_neg)) | 
       !base::all(is.na(sizer_data$neg_to_pos))){
      p <- p + ggplot2::geom_smooth(ggplot2::aes(group = groups, 
                                                 color = 'static'), method = stats::lm, formula = "y ~ x", 
                                    se = FALSE) + ggplot2::scale_color_manual(values = sharp_colors[2]) + 
        ggplot2::theme(legend.position = "none") }
    
    # If trendline is sharp but no inflections exist, use both colors
    if(trendline == "sharp" & base::all(is.na(sizer_data$pos_to_neg)) & 
       base::all(is.na(sizer_data$neg_to_pos))){
      p <- p + ggplot2::geom_smooth(ggplot2::aes(group = groups, color = slope_type), method = stats::lm, formula = "y ~ x", 
                                    se = FALSE) + ggplot2::scale_color_manual(values = sharp_colors) + 
        ggplot2::theme(legend.position = "none") }
    
    return(p)
  }
  
  sizer_ggplot_test(raw_data = data_info,
               sizer_data = sizer_info,
               x = explanatory_var, y = response_var,
               trendline = 'sharp', vline = "inflections",
               sharp_colors = c("#bbbbbb", "green")) +
