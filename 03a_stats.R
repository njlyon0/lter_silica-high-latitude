...
## ----------------------------------------- ##
# % Change Nutrient Conc Response within LTER Across Months ----
## ----------------------------------------- ##
...
#comment out files not using
month_v1 <- read.csv(file = file.path("data", "stats-ready_monthly", "stats-ready_monthly_Conc_uM_DSi.csv"))
#month_v1 <- read.csv(file = file.path("data", "stats-ready_monthly", "stats-ready_monthly_Conc_uM_DIN.csv"))
#month_v1 <- read.csv(file = file.path("data", "stats-ready_monthly", "stats-ready_monthly_Conc_uM_P.csv"))
...
month_v2 <- month_v1 %>% 
  ...
  dplyr::select(-slope_estimate, -slope_direction, -slope_std_error,
                ...
                -dplyr::contains("_Si.DIN_"), -dplyr::contains("_Si.P_")) %>% 
  # Change certain column names to be more informative
  dplyr::rename(mean_conc = mean_response,
                perc.change_conc = percent_change) %>% 
...
for(ltername in unique(month_v2$LTER)){
  ...
  # Fit linear model
  aov_perc.change_month <- lm(perc.change_conc ~ Stream_Name + as.factor(Month),
                              data = month_sub)
...
write.csv(x = month_out, row.names = F, na = '',
          file = file.path("data", "stats-results", "monthly_perc_change_P_results.csv"))


## ----------------------------------------- ##
# % Change Nutrient Yields Response within LTER Across Months ----
## ----------------------------------------- ##
...
# Read in monthly (prepared) output
#comment out files not using
#month_v1 <- read.csv(file = file.path("data", "stats-ready_monthly", "stats-ready_monthly_Yield_DSi.csv"))
#month_v1 <- read.csv(file = file.path("data", "stats-ready_monthly", "stats-ready_monthly_Yield_DIN.csv"))
month_v1 <- read.csv(file = file.path("data", "stats-ready_monthly", "stats-ready_monthly_Yield_P.csv"))
...
month_v2 <- month_v1 %>% 
  ...
  dplyr::select(-slope_estimate, -slope_direction, -slope_std_error,
                ...
                -dplyr::contains("_Si.DIN_"), -dplyr::contains("_Si.P_")) %>% 
  # Change certain column names to be more informative
  dplyr::rename(mean_yield = mean_response,
                perc.change_yield = percent_change) %>% 
  #drop MCM sites not longer using
  dplyr::filter(!LTER_stream %in% c("MCM_Commonwealth S", "MCM_Crescent Strea", 
                                    "MCM_Delta Stream  ", "MCM_Harnish Creek ",
                                    "MCM_Onyx River  La", "MCM_Onyx River  Lo",
...
for(ltername in unique(month_v2$LTER)){
  ...
  # Fit linear model
  aov_perc.change_month <- lm(perc.change_yield ~ Stream_Name + as.factor(Month),
                              data = month_sub)
...
#change file name accordingly
write.csv(x = month_out, row.names = F, na = '',
          file = file.path("data", "stats-results", "monthly_perc_change_P_Yields_results.csv"))


## ----------------------------------------- ##
# % Change Discharge Response within LTER Across Months ----
## ----------------------------------------- ##

# Read in monthly (prepared) output
month_v1 <- read.csv(file = file.path("data", "stats-ready_monthly", "stats-ready_monthly_Discharge_cms_DSi.csv"))
...
month_v2 <- month_v1 %>% 
  ...
  dplyr::select(-slope_estimate, -slope_direction, -slope_std_error,
                ...
                -dplyr::contains("_Si.DIN_"), -dplyr::contains("_Si.P_")) %>% 
  # Change certain column names to be more informative
  dplyr::rename(mean_Q = mean_response,
                perc.change_Q = percent_change) %>% 
  #drop MCM sites not longer using
  dplyr::filter(!LTER_stream %in% c("MCM_Commonwealth S", "MCM_Crescent Strea", 
                                    "MCM_Delta Stream  ", "MCM_Harnish Creek ",
                                    "MCM_Onyx River  La", "MCM_Onyx River  Lo",
...
for(ltername in unique(month_v2$LTER)){
  ...
  # Fit linear model
  aov_perc.change_month <- lm(perc.change_Q ~ Stream_Name + as.factor(Month),
                              data = month_sub)
  ...
  # Generate / export a residual plot (to evaluate model fit)
  ggResidpanel::resid_panel(model = aov_perc.change_month)
  ggplot2::ggsave(filename = file.path("data", "stats-results", paste0("monthly_perc_change_Discharge_residuals_", ltername, ".png")),
                  height = 5, width = 5, units = "in")
...
# Export results
write.csv(x = month_out, row.names = F, na = '',
          file = file.path("data", "stats-results", "monthly_perc_change_Discharge_results.csv"))




## ----------------------------------------- ##
# % Change in Annual Si Response across LTERs ----
## ----------------------------------------- ##  
